...
if (!$csv) {
        echo "<table>\n";
        echo "<tr><th>Server Name</th><th>IP Addr</th><th>Updates</th><th>Release</th><th>Last Checkin</th><th>Updated</th><th>Actions</th></tr><br/>\n";
} else {
        $csvrow = array('Server Name','IP Addr','Bug Updates','Security Updates','Total Updates','Release','Last Checkin');
        fputcsv($csvh, $csvrow);
}
if ($dbresult && mysqli_num_rows($dbresult)) {
        while ($row = mysqli_fetch_object($dbresult)) {
                ...
                if ($csv) {
                        $csvrow = array($row->hostname,$row->ipaddr,$updatecounts['UPDATE'],$updatecounts['SECURITY'],$updatecounts['TOTAL'],$row->distrorelease,$row->checkin);
                        fputcsv($csvh, $csvrow);
                } else {
                        echo "<tr><td>$row->hostname</td><td>$row->ipaddr</td>";
                        ...
                        echo "</table></td>";
                        if($row->updated == 'yes') {
                                echo "<td>$row->distrorelease</td><td>$row->checkin</td><td width='2%'><i class='fa fa-check good' aria-hidden='true'></td><td>";
                        }else{
                                echo "<td>$row->distrorelease</td><td>$row->checkin</td><td width='2%'><i class='fa fa-times bad' aria-hidden='true'></td><td>";
                        }
